...
def inference_mfcc(args, mfcc, sample_fn, model, n_frames=0, smoothing=False, SG_filter=False, minibatch=False, skip_timesteps=0, n_seed=8, style=None, seed=123456, smooth_foot=False, device=torch.device('cuda:0')):
    ...
    model_kwargs_ = {'y': {}}
    model_kwargs_['y']['mask'] = (torch.zeros([1, 1, 1, n_frames]) < 1).to(device)
    model_kwargs_['y']['style'] = torch.as_tensor([style]).float().to(device)
    model_kwargs_['y']['mask_local'] = torch.ones(1, args.n_poses).bool().to(device)
    ...
    if minibatch:
        audio_reshape = torch.from_numpy(mfcc).to(torch.float32).reshape(num_subdivision, stride_poses, -1).to(device).permute(1, 0, 2)  # mfcc[:, :-2]
        ...
        for i in range(0, num_subdivision):
            ...
            if i == 0:
                if n_seed != 0:
                    pad_zeros = torch.zeros([n_seed, 1, 13]).to(device)  # mfcc dims are 13
                    model_kwargs_['y']['audio'] = torch.cat((pad_zeros, model_kwargs_['y']['audio']), 0)
                    model_kwargs_['y']['seed'] = torch.zeros([1, 1141, 1, n_seed]).to(device)
            else:
                if n_seed != 0:
        ...
        model_kwargs_['y']['audio'] = torch.from_numpy(mfcc).to(torch.float32).unsqueeze(0).to(device).permute(1, 0, 2)
...
def inference(args, wavlm_model, audio, sample_fn, model, n_frames=0, smoothing=False, SG_filter=False, minibatch=False, skip_timesteps=0, n_seed=8, style=None, seed=123456, device=torch.device('cuda:0')):
    ...
    if minibatch:
        audio_reshape = torch.from_numpy(audio).to(torch.float32).reshape(num_subdivision, int(stride_poses * 16000 / 20)).to(device).transpose(0, 1)  # mfcc[:, :-2]
        shape_ = (1, model.njoints, model.nfeats, args.n_poses)
        out_list = []
        for i in range(0, num_subdivision):
            print(i, num_subdivision)
            ...
            if i == 0:
                if n_seed != 0:
                    pad_zeros = torch.zeros([int(n_seed * 16000 / 20), 1]).to(device)  # wavlm dims are 1024
                    model_kwargs_['y']['audio'] = torch.cat((pad_zeros, model_kwargs_['y']['audio']), 0)
                    model_kwargs_['y']['seed'] = torch.zeros([1, 1141, 1, n_seed]).to(device)
            else:
                if n_seed != 0:
        ...
        model_kwargs_['y']['audio'] = torch.from_numpy(audio).to(torch.float32).unsqueeze(0).to(device).permute(1, 0, 2)
        shape_ = (batch_size, model.njoints, model.nfeats, n_frames)
        model_kwargs_['y']['seed'] = torch.zeros([1, 1141, 1, n_seed]).to(device)
        sample = sample_fn(
            model,
...
def main(args, save_dir, model_path, audio_path=None, mfcc_path=None, speech_path=None, max_len=0, device=torch.device('cuda:0')):
    ...
    if speech_path != None:
        ...
        raise ValueError('Please provide either mfcc or mfcc_path')
    ...
    wavlm_model = wavlm_init(device=device)
    inference(args, wavlm_model, mfcc, sample_fn, model, n_frames=max_len, smoothing=True, SG_filter=True, minibatch=True, skip_timesteps=0, style=style, seed=123456, device=device)  # style2onehot['Happy']


if __name__ == '__main__':
    """
    cd diffuse_style_gesture
    python sample.py --config=./configs/DiffuseStyleGesture.yml --gpu mps --model_path=./model000450000.pt --speech_path=./003_Neutral_2_x_1_0.wav
    """
    # prefix = str(datetime.now().strftime('%Y%m%d_%H%M%S'))
    ...
    parser.add_argument('--speech_folder', type=str, default=None)
    ...
    # if args.speech_folder is not None:
    #     wave_files = [os.path.join(args.speech_folder, wav) for wav in sorted(os.listdir(args.speech_folder)) if wav.endswith('.wav')]
    #
    #     for wav_file in wave_files:
    #         print("Sampling .. ", wav_file)
    #         main(config, save_dir, config.model_path, audio_path=None, mfcc_path=None, speech_path=wav_file, max_len=config.max_len, device=device)
