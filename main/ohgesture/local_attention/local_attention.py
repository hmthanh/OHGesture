...
class LocalAttention(nn.Module):
    ...
    def forward(self, q, k, v, packed_shape, mask=None, input_mask=None):
        ...
        # (q, packed_shape), (k, _), (v, _) = map(lambda t: pack([t], '* n d'), (q, k, v))        # (2, 8, 2048, 64) -> (16, 2048, 64)

        # rotary embeddings

        # if exists(self.rel_pos):
        #     pos_emb = self.rel_pos(q)     # (16, 2048, 64)
        ...
        if n % window_size != 0:
            print('sequence length must be divisible by window size for local attention', n, window_size)
            assert True
